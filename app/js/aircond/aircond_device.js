...
var endDate;
var table;
App.controller('aircondDeviceBarController', ['$scope', function ($scope) {
    $scope.menus = menuItems;
}]);
var device_type="qichuang";//气调
App.controller('aircondDeviceController', ['$scope', '$http', 'ngDialog', '$filter', function ($scope, $http, ngDialog, $filter) {
    $scope.gridtableid = "aircondDeviceTableGrids"; //数据列表id
    ...
    $scope.deviceTypeRemark = function () {
        //获取节点配置的信息
        $.ajax({
            url: "/la/device/remark?typecode=" + device_type,
            method: 'POST',
            async: false
...
App.controller('aircondDeviceViewController', ['$scope', '$http', 'ngDialog', '$filter', function ($scope, $http, ngDialog, $filter) {
    $scope.deviceView();
}]);


//列表信息处理
App.controller('aircondDeviceGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
