...
var endDate;
var table;
App.controller('aircondHistoryBarController', ['$scope', function ($scope) {
    $scope.menus = menuItems;
}]);
App.controller('aircondHistoryController', ['$scope', '$http', 'ngDialog', '$filter', function ($scope, $http, ngDialog, $filter) {
    $scope.gridtableid = "aircondHistoryTableGrids"; //数据列表id
...
//列表信息处理
App.controller('aircondHistoryGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    ...
    table = $('#' + $scope.gridtableid).DataTable({
        ...
        ajaxSource: "/aircond/list",//获取数据的url
        ...
        aoColumns: [
            ...
            {
                mDataProp: "mode", sTitle: "气调方式"
            },
            {
                mDataProp: "timelength", sTitle: "气调时长", render: function (data) {
                return data + "H";
            }
            },
            {
    ...
    $("#" + $scope.gridtableid + " tbody").on('click', 'tr td button.toView', function () {
        ...
        window.location.href = "#/aircond/aircond_view/" + data.id + "/" + selHouseName + "/" + data.crtime;
    });
}]);
