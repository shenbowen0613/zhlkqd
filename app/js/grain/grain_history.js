...
var table;
var startDate;
App.controller('grainHistoryBarController', ['$scope', function ($scope) {
    $scope.menus = menuItems;
}]);
App.controller('grainHistoryController', ['$scope', '$http', 'ngDialog', '$filter', function ($scope, $http, ngDialog, $filter) {
    $scope.gridtableid = "grainHistoryTableGrids"; //数据列表id
    $scope.startDate = $filter('date')(startDate, 'yyyy-MM-dd'); //格式化开始日期;
    ...
    var pData = {code: selHouseCode, date: startDate};
    $.ajax({
                ...
                if (result.success) {
                    ...
                    if ($scope.tempData.aaData == null || $scope.tempData.aaData == "") {
                        ...
                        table = $('#' + $scope.gridtableid).DataTable({
                            ...
                            bPaginate: false, pageLength: 20,                    //每页显示20条数据
                            data: $scope.tempData.aaData,
                            aoColumns: [
                                //{mDataProp: "id", sTitle: "id"},
                                {sTitle: "序号", width: "8%"},
...
//列表信息处理
App.controller('grainHistoryGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    var operatehtml = getHtmlInfos("app/views/base/grid_details.html", "查看详情", "toViewTemp");
    //数据列表信息
    table = $('#' + $scope.gridtableid).DataTable({
        searching: false,                      //不显示搜索框
