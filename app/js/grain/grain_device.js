...
var endDate;
var table;
App.controller('grainDeviceBarController', ['$scope', function ($scope) {
    $scope.menus = menuItems;
}]);
var device_type="cewen";//测温 默认
App.controller('grainDeviceController', ['$scope', '$http', 'ngDialog', '$filter', function ($scope, $http, ngDialog, $filter) {
    $scope.gridtableid = "grainDeviceTableGrids"; //数据列表id
...
//列表信息处理
App.controller('grainDeviceGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
    ...
    table = $('#' + $scope.gridtableid).DataTable({
        ...
        ajaxSource: GserverURL+"la/device/glist",//获取数据的url
        ...
        aoColumns: [
            ...
            },
            {
                ...
                return data?'<font color="red">运行</font>':'停止';
            }
            }
        ],
        aoColumnDefs: [//设置列的属性
...
App.controller('grainDeviceViewController', ['$scope', '$http', 'ngDialog', '$filter', function ($scope, $http, ngDialog, $filter) {
    $scope.deviceView();
}]);
