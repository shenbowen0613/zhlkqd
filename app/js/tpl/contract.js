...
App.controller('contracttplController', ['$scope', '$http', "ngDialog", function ($scope, $http, ngDialog) {
    global_type_code = null;
    $scope.gridtableid="contracttplTableGrids";
    //获取类型树信息
    $.ajax({ //查询按钮权限
        url: GserverURL+"/sys/dict/list?typecode=contracttype",
        method: 'POST',
        async: false
    ...
    //获取类型树信息
    $.ajax({ //查询按钮权限
        url: "/tpl/contract/tpls?tplname=contract",
        method: 'POST',
        async: false
    }).success(function (response) {
        if (response.success) {
    ...
    $scope.toRemove=function(){
        window.location.href = "#/app/tpl/contract";
    }
    //绑定topbar操作
    function topbar_bind_operate(ngDialog){
        ...
        $("#toadd").click(function () {
            ...
            window.location.href = "#/app/tpl/contract/add";
        });
        ...
        $("#toupdate").click(function () {
            ...
            window.location.href = "#/app/tpl/contract/update";
            $("#fileurl").val(contract.fileurl); //
        });
        ...
        $("#todelete").click(function () {
            ...
            ngDialog.openConfirm({
                template: 'delcontracttplDialogId',
                className: 'ngdialog-theme-default'
            }).then(function (value) { //用户点击确认
                $http({
                    url: '/tpl/contract/delete',
                    method: 'POST',
                    data: {id: contract.id}
...
//类型树信息处理
App.controller('contracttplTypeController', ['$scope', "$http", function ($scope, $http) {
    $scope.show_all_info = true;
    $scope.all_info = "全部类型";
    $scope.showAllInfo = function(){
        global_type_code=null;
        $scope.toRemove();
        $("#contracttplTypePanel").find("li").removeClass("active"); //清除已选中
        var table = $('#'+$scope.gridtableid).DataTable();
        table.draw();
        ckClickTr($scope.gridtableid); //单击行，选中复选框
    }
...
//列表信息处理
App.controller('contracttplGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "/tpl/contract/list",//获取数据的url
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/tpl/contract/view/"+data.id;
    });
}]);
// 查看详情
App.controller("viewcontracttplController", function ($scope, $stateParams,$http, ngDialog) {


    $http({
            url: GserverURL+ '/tpl/contract/view',
        method: 'POST',
        ...
        if (response.success) { //信息处理成功，进入用户中心页面
            ...
            angular.forEach($scope.treeInfos, function (data, index, array) {
                if($scope.contract.typecode==data.code){
                    $scope.contract.typename=data.label;
                }
            });
...
//添加信息
App.controller("addcontracttplController", function ($scope, $http, ngDialog) {
    $scope.contract = null;
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
           ...
           $http({
                url: GserverURL+'/tpl/contract/add',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
...
//修改信息
App.controller("updatecontracttplController", function ($scope, $http, ngDialog) {
    ...
    $scope.save = function () {
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
           ...
           $http({
                url: GserverURL+'/tpl/contract/update',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
