...
App.controller('aerationtplController', ['$scope', '$http', "ngDialog", function ($scope, $http, ngDialog) {
    global_type_code = null;
    $scope.gridtableid="aerationtplTableGrids";
    //获取类型树信息
    $.ajax({ //查询按钮权限
        url: GserverURL+"/sys/dict/list?typecode=aerationtype",
        method: 'POST',
        async: false
    ...
    $scope.toRemove=function(){
        window.location.href = "#/app/tpl/aeration";
    }
    //绑定topbar操作
    function topbar_bind_operate(ngDialog){
        ...
        $("#toadd").click(function () {
            ...
            window.location.href = "#/app/tpl/aeration/add";
        });

        //修改
        $("#toupdate").click(function () {
            var aeration=getTableData(table);
            if(aeration==null){
                rzhdialog(ngDialog,"请先选择一条要修改的数据！","error");
                return;
            }
            $scope.aeration=aeration;
            window.location.href = "#/app/tpl/aeration/update";
            $("#typecode").val(aeration.typecode); //
        });
        ...
        $("#todelete").click(function () {
            ...
            ngDialog.openConfirm({
                template: 'delaerationtplDialogId',
                className: 'ngdialog-theme-default'
            }).then(function (value) { //用户点击确认
                $http({
                    url: '/tpl/aeration/delete',
                    method: 'POST',
                    data: {id: aeration.id}
        ...
        $("#toconfig").click(function(){
            ...
            window.location.href="#/app/tpl/aerationcfg/"+aeration.id;
        });
    }
...
//类型树信息处理
App.controller('aerationtplTypeController', ['$scope', "$http", function ($scope, $http) {
    $scope.show_all_info = true;
    $scope.all_info = "全部类型";
    $scope.showAllInfo = function(){
        global_type_code=null;
        $scope.toRemove();
        $("#aerationtplTypePanel").find("li").removeClass("active"); //清除已选中
        var table = $('#'+$scope.gridtableid).DataTable();
        table.draw();
        ckClickTr($scope.gridtableid); //单击行，选中复选框
    }
...
//列表信息处理
App.controller('aerationtplGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "/tpl/aeration/list",//获取数据的url
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/tpl/aeration/view/"+data.id;
    });
}]);
// 查看详情
App.controller("viewaerationtplController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
            url: GserverURL+ '/tpl/aeration/view',
        method: 'POST',
        data: {id: $stateParams.id}
    }).success(function (response) { //提交成功
        if (response.success) { //信息处理成功，进入用户中心页面
            $scope.aeration = response.data;
            angular.forEach($scope.treeInfos, function (data, index, array) {
                if($scope.aeration.type==data.code){
                    $scope.aeration.typename=data.label;
                }
            });
            $scope.aeration.crtime=formatDate( $scope.aeration.crtime);
            $scope.aeration.uptime=formatDate( $scope.aeration.uptime);
            $scope.aeration.isdefault?$scope.aeration.isdefault="是":$scope.aeration.isdefault="否";

            //格式化步骤
            var workdetail=$scope.aeration.workdetail;
            if(workdetail==null||workdetail==""){
                $scope.aeration.workdetail=[];
                return;
            }
            $scope.aeration.workdetail=eval(workdetail);
        } else { //信息处理失败，提示错误信息
            rzhdialog(ngDialog,response.info,"error");
        }
    }).error(function (response) { //提交失败
...
//添加信息
App.controller("addaerationtplController", function ($scope, $http, ngDialog) {
    $scope.aeration = null;
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            //增加分类属性
            $scope.aeration.type=global_type_code;
            var pData = {
                paras: JSON.stringify($scope.aeration)
            };
           $http({
                url: GserverURL+'/tpl/aeration/add',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
...
//修改信息
App.controller("updateaerationtplController", function ($scope, $http, ngDialog) {
    ...
    $scope.save = function () {
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
           ...
           $http({
                url: GserverURL+'/tpl/aeration/update',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
