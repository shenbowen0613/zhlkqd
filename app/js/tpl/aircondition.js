...
App.controller('airconditiontplController', ['$scope', '$http', "ngDialog", function ($scope, $http, ngDialog) {
    global_type_code = null;
    $scope.gridtableid="airconditiontplTableGrids";
    //获取类型树信息
    $.ajax({ //查询按钮权限
        url: GserverURL+"/sys/dict/list?typecode=airconditiontype",
        method: 'POST',
        async: false
    ...
    $scope.toRemove=function(){
        window.location.href = "#/app/tpl/aircondition";
    }
    //绑定topbar操作
    function topbar_bind_operate(ngDialog){
        ...
        $("#toadd").click(function () {
            ...
            window.location.href = "#/app/tpl/aircondition/add";
        });

        //修改
        $("#toupdate").click(function () {
            var aircondition=getTableData(table);
            if(aircondition==null){
                rzhdialog(ngDialog,"请先选择一条要修改的数据！","error");
                return;
            }
            $scope.aircondition=aircondition;
            window.location.href = "#/app/tpl/aircondition/update";
            $("#typecode").val(aircondition.typecode); //
        });
        ...
        $("#todelete").click(function () {
            ...
            ngDialog.openConfirm({
                template: 'delairconditiontplDialogId',
                className: 'ngdialog-theme-default'
            }).then(function (value) { //用户点击确认
                $http({
                    url: '/tpl/aircondition/delete',
                    method: 'POST',
                    data: {id: aircondition.id}
        ...
        $("#toconfig").click(function(){
            ...
            window.location.href="#/app/tpl/airconditioncfg/"+aircondition.id;
        });
    }
...
//类型树信息处理
App.controller('airconditiontplTypeController', ['$scope', "$http", function ($scope, $http) {
    $scope.show_all_info = true;
    $scope.all_info = "全部类型";
    $scope.showAllInfo = function(){
        global_type_code=null;
        $scope.toRemove();
        $("#airconditiontplTypePanel").find("li").removeClass("active"); //清除已选中
        var table = $('#'+$scope.gridtableid).DataTable();
        table.draw();
        ckClickTr($scope.gridtableid); //单击行，选中复选框
    }
...
//列表信息处理
App.controller('airconditiontplGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "/tpl/aircondition/list",//获取数据的url
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/tpl/aircondition/view/"+data.id;
    });
}]);
// 查看详情
App.controller("viewairconditiontplController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
            url: GserverURL+ '/tpl/aircondition/view',
        method: 'POST',
        data: {id: $stateParams.id}
    }).success(function (response) { //提交成功
        if (response.success) { //信息处理成功，进入用户中心页面
            $scope.aircondition = response.data;
            angular.forEach($scope.treeInfos, function (data, index, array) {
                if($scope.aircondition.type==data.code){
                    $scope.aircondition.typename=data.label;
                }
            });
            $scope.aircondition.crtime=formatDate( $scope.aircondition.crtime);
            $scope.aircondition.uptime=formatDate( $scope.aircondition.uptime);
            $scope.aircondition.isdefault?$scope.aircondition.isdefault="是":$scope.aircondition.isdefault="否";
            //格式化步骤
            var workdetail=$scope.aircondition.workdetail;
            if(workdetail==null||workdetail==""){
                $scope.aircondition.workdetail=[];
                return;
            }
            $scope.aircondition.workdetail=eval("("+workdetail+")");
        } else { //信息处理失败，提示错误信息
            rzhdialog(ngDialog,response.info,"error");
        }
    }).error(function (response) { //提交失败
...
//添加信息
App.controller("addairconditiontplController", function ($scope, $http, ngDialog) {
    $scope.aircondition = null;
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        var table = $('#'+$scope.gridtableid).DataTable();
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            //增加分类属性
            $scope.aircondition.type=global_type_code;
            var pData = {
                paras: JSON.stringify($scope.aircondition)
            };
           $http({
                url: GserverURL+'/tpl/aircondition/add',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
...
//修改信息
App.controller("updateairconditiontplController", function ($scope, $http, ngDialog) {
    ...
    $scope.save = function () {
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
           ...
           $http({
                url: GserverURL+'/tpl/aircondition/update',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
