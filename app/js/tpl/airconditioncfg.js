...
App.controller('airconditioncfgController', ['$scope', '$http',"$stateParams", "ngDialog", function ($scope, $http, $stateParams, ngDialog) {
    ...
    $scope.gridtableid="airconditioncfgTableGrids";
    //获取类型树信息
    $.ajax({ //查询按钮权限
        url: GserverURL+"/sys/dict/list?typecode=qttaskparas",
        method: 'POST',
        async: false
    ...
    $scope.toRemove=function(){
        window.location.href = "#/app/tpl/airconditioncfg/"+$scope.tplid;
    }
    //绑定topbar操作
    //添加
    $("#toadd").click(function () {
        window.location.href = "#/app/tpl/airconditioncfg/"+$scope.tplid+"/add";
    });
    ...
    $("#toupdate").click(function () {
        ...
        var airconditioncfg=getTableData(table);
        if(airconditioncfg==null){
            rzhdialog(ngDialog,"请先选择一条要修改的数据！","error");
            return;
        }
        $scope.airconditioncfg=airconditioncfg;
        $scope.oldno=airconditioncfg.no;
        window.location.href = "#/app/tpl/airconditioncfg/"+$scope.tplid+"/update";
        $("#user").val(airconditioncfg.user);
    });
    ...
    $("#todelete").click(function () {
        ...
        ngDialog.openConfirm({
            template: 'delairconditioncfgDialogId',
            className: 'ngdialog-theme-default'
        }).then(function (value) { //用户点击确认
           $http({
                url: GserverURL+'/tpl/aircondition/paradelete',
                method: 'POST',
                data: {id:$scope.tplid ,no: airconditioncfg.no}
...
//类型树信息处理
//App.controller('airconditioncfgTypeController', ['$scope', "$http", function ($scope, $http) {
...
//列表信息处理
App.controller('airconditioncfgGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "/tpl/aircondition/paralist",//获取数据的url
...
//添加信息
App.controller("addairconditioncfgController", function ($scope, $http, ngDialog) {
    $scope.airconditioncfg = null;
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        var table = $('#'+$scope.gridtableid).DataTable();
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            ...
            angular.forEach($scope.treeInfos, function (data, index, array) {
                if($scope.airconditioncfg.type==data.code){
                    $scope.airconditioncfg.name=data.label;
                }
            });
            var pData = {
                id:$scope.tplid,
                paras: JSON.stringify($scope.airconditioncfg)
            };
           $http({
                url: GserverURL+'/tpl/aircondition/paraadd',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
