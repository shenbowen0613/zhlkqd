...
App.controller('custController', ['$scope', '$http', "ngDialog", function ($scope, $http, ngDialog) {
    global_type_code = null;
    $scope.gridtableid="custTableGrids";
    $scope.global_type=0;//默认为支出
    $scope.global_treetype="cust"+$scope.global_type;
    ...
    function topbar_bind_operate(ngDialog){
        ...
        $("#toadd").click(function () {
            ...
            window.location.href = "#/app/buss/cust/add";
        });

        //修改
        $("#toupdate").click(function () {
            var cust=getTableData(table);
            if(cust==null){
                rzhdialog(ngDialog,"请先选择一条要修改的数据！","error");
                return;
            }
            $scope.cust=cust;
            if($scope.cust.startime!=null&&$scope.cust.startime!=''){
                $scope.cust.startime=formatDay( $scope.cust.startime);
            }
            if($scope.cust.endtime!=null&&$scope.cust.endtime!=''){
                $scope.cust.endtime=formatDay( $scope.cust.endtime);
            }
            window.location.href = "#/app/buss/cust/update";
        });
        ...
        $("#todelete").click(function () {
            ...
            ngDialog.openConfirm({
                template: 'delcustDialogId',
                className: 'ngdialog-theme-default'
            }).then(function (value) { //用户点击确认
                $http({
                    url: '/buss/cust/delete',
                    method: 'POST',
                    data: {id: cust.id}
        ...
        $("#toreport").click(function(){
            window.location.href = "#/app/report/buss"+"/app-buss-cust";
            return;
        });
    }
    //关闭弹出窗口操作
    $scope.toRemove=function(){
        window.location.href = "#/app/buss/cust";
    }
...
//类型树信息处理
App.controller('custTypeController', ['$scope', "$http", function ($scope, $http) {
    $scope.show_all_info = true;
    $scope.all_info = "全部类型";
    $scope.showAllInfo = function(){
        global_type_code=null;
        $scope.toRemove();
        $("#custTypePanel").find("li").removeClass("active"); //清除已选中
        var table = $('#'+$scope.gridtableid).DataTable();
        table.draw();
        ckClickTr($scope.gridtableid); //单击行，选中复选框
    }
...
//列表信息处理
App.controller('custGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "buss/cust/list",//获取数据的url
        ...
        aoColumns: [
            ...
            //{mDataProp: "shortname", sTitle: "简称"},
            //{mDataProp: "address", sTitle: "地址"},
            {mDataProp: "products", sTitle: "产品"},
            {mDataProp: "legalperson", sTitle: "法人"},
            {mDataProp: "phone", sTitle: "座机"},
            //{mDataProp: "mobile", sTitle: "手机"},
            //{mDataProp: "fax", sTitle: "传真"},
            //{mDataProp: "email", sTitle: "邮件"},
            {mDataProp: "startime",sTitle: "合作开始时间",render:function(data){return formatDay(data)}},
            {sTitle: "操作"}
        ],
        aoColumnDefs: [//设置列的属性
            {
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/buss/cust/view/"+data.id;
    });
}]);
// 查看详情
App.controller("viewCustController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
            url: GserverURL+ '/buss/cust/view',
        method: 'POST',
...
//添加信息
App.controller("addCustController", function ($scope, $http, ngDialog,$filter) {
    $scope.cust = null;//初始化数据
    ...
    $scope.save = function () {
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            //增加分类属性
            $scope.cust.typecode=global_type_code;
            $scope.cust.type=$scope.global_type;
            if($scope.cust.startime!=null&&$scope.cust.startime!=''){
                $scope.cust.startime = formatDayNum8($scope.cust.startime);
            }
            if($scope.cust.endtime!=null&&$scope.cust.endtime!=''){
                $scope.cust.endtime = formatDayNum8($scope.cust.endtime);
            }

            var pData = {
                paras: JSON.stringify($scope.cust)
            };
           $http({
                url: GserverURL+'/buss/cust/add',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
...
//修改信息
App.controller("updateCustController", function ($scope, $http, $filter, ngDialog) {
    $('#endtime,#startime').datetimepicker({ //加载日期插件
        todayBtn:  1, // '今天'按钮显示
        autoclose: 1, //选择完成自动关闭
        minView: 2, //最小显示单位  2 代表到天
    ...
    $scope.save = function () {
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
           ...
           $http({
                url: GserverURL+'/buss/cust/update',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
