...
App.controller('lataskrecController', ['$scope', '$http',"$stateParams", "ngDialog", function ($scope, $http,$stateParams, ngDialog) {
    $scope.taskcode = $stateParams.taskcode;
    $scope.taskname= $stateParams.taskname;
    global_type_code = null;
    $scope.gridtableid="lataskrecTableGrids";
    ...
    //    windows.location.href = "#/app/buss/latask_rec/"+$scope.no+"/"+$scope.lataskname+"/add";
    //});
...
//列表信息处理
App.controller('lataskrecGridController', ['$scope', "$http", "ngDialog", function ($scope,$stateParams, $http, ngDialog) {
    //分页动态加载数据
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/buss/latask_rec/"+$scope.taskcode+"/"+$scope.taskname+"/view/"+data.id;
    });
}]);
// 查看详情
App.controller("viewlataskrecController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
        ...
        if (response.success) { //信息处理成功，进入用户中心页面
            $scope.lataskrec = response.data;
            $scope.lataskrec.starttime=formatDate( $scope.lataskrec.starttime);
            $scope.lataskrec.endtime=formatDate( $scope.lataskrec.endtime);
        } else { //信息处理失败，提示错误信息
