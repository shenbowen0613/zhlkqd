...
App.controller('contractController', ['$scope', '$http', "ngDialog", function ($scope, $http, ngDialog) {
    global_type_code = null;
    $scope.gridtableid="contractTableGrids";
    //获取类型树信息
    $.ajax({ //查询按钮权限
        url: GserverURL+"/sys/dict/list?typecode=contractplan",
        method: 'POST',
        async: false
    ...
    //获取类型树信息
    $.ajax({ //查询按钮权限
        url: "/tpl/approval/tree?typecode=contractplan",
        method: 'POST',
        async: false
    }).success(function (response) {
        if (response.success) {
    ...
    //获取类型树信息
    $.ajax({ //查询按钮权限
        url: GserverURL+"/sys/dict/list?typecode=contractstatus",
        method: 'POST',
        async: false
    }).success(function (response) {
        if (response.success) {
    ...
    $scope.toRemove=function(){
        window.location.href = "#/app/buss/contract";
    }
    //绑定topbar操作
    function topbar_bind_operate(ngDialog){
        ...
        $("#toadd").click(function () {
            ...
            //根据类型加载模板函数
            $scope.loadcontracttpl(global_type_code);
            window.location.href = "#/app/buss/contract/add";
        });

        //修改
        $("#toupdate").click(function () {
            var contract=getTableData(table);
            if(contract==null){
                rzhdialog(ngDialog,"请先选择一条要修改的数据！","error");
                return;
            }
            if(contract.statuscode!=0&&contract.statuscode!=-1){
                rzhdialog(ngDialog,'只有“已创建”和“已取消”状态可以修改！',"error");
                return;
            }
            $scope.contract=contract;
            if($scope.contract.startime!=null&&$scope.contract.startime!=''){
                $scope.contract.startime=formatDay( $scope.contract.startime);
            }
            if($scope.contract.endtime!=null&&$scope.contract.endtime!=''){
                $scope.contract.endtime=formatDay( $scope.contract.endtime);
            }
            //根据类型加载模板函数
            $scope.loadcontracttpl($scope.contract.typecode);
            window.location.href = "#/app/buss/contract/update";
            $("#sourcecode").val(contract.sourcecode); //
            $("#statuscode").val(contract.statuscode); //
            $("#priority").val(contract.priority); //
        });
        ...
        $("#tochange").click(function(){
            ...
            //获取类型树信息
            $.ajax({ //查询按钮权限
                url: "/buss/plan/status?id="+contract.id,
                method: 'POST',
                async: false
            }).success(function (response) {
                if (response.success) {
                    $scope.statustreeInfos = response.data;
                    window.location.href = "#/app/buss/contract/change";
                }else{
                    rzhdialog(ngDialog,response.info,"error");
                }
            });
        ...
        $("#toapproval").click(function(){
            ...
            window.location.href="#/app/buss/approval/contract/"+contract.code;
        });
        //上传附件
        $("#toupload").click(function(){
            ...
            window.location.href="#/app/buss/attach/contract/"+contract.code;
        });

        //生成报表
        $("#toreport").click(function(){
            window.location.href = "#/app/report/buss"+"/app-buss-contract";
            return;
        });
    }

    //根据类型加载模板函数
    $scope.loadcontracttpl=function(typecode){
        //获取类型树信息
        $.ajax({ //查询按钮权限
            url: "/tpl/contract/tree?typecode="+typecode,
            method: 'POST',
            async: false
        }).success(function (response) {
            if (response.success) {
                $scope.tplcontracttreeInfos = response.data;
            }
        });
    }
}]);
...
//类型树信息处理
App.controller('contractTypeController', ['$scope', "$http", function ($scope, $http) {
...
//列表信息处理
App.controller('contractGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
    function retrieveData(sSource, aoData, fnCallback) {
        ...
        $.ajax({
            ...
            url: sSource,
            dataType: 'json',
            data: {"aoData": JSON.stringify(aoData),"type":"contract","typecode":global_type_code}, //以json格式传递
            async: false,
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/buss/contract/view/"+data.id;
    });
    //文件下载
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.todownload', function () {
        var data = table.row($(this).parent().parent()).data(); //获取爷爷节点 tr 的值
        openfile(ngDialog,data.attach);
    });
}]);
// 查看详情
App.controller("viewcontractController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
        ...
        if (response.success) { //信息处理成功，进入用户中心页面
            ...
            angular.forEach($scope.treeInfos, function (data, index, array) {
                if($scope.contract.typecode==data.code){
                    $scope.contract.typecode=data.label;
                }
            });
            angular.forEach($scope.prioritytreeInfos, function (data, index, array) {
                if($scope.contract.priority==data.code){
                    $scope.contract.priority=data.label;
                }
            });
            if($scope.contract.startime!=null&&$scope.contract.startime!=''){
                $scope.contract.startime=formatDay( $scope.contract.startime);
            ...
            $scope.contract.crtime=formatDate( $scope.contract.crtime);
            $scope.contract.uptime=formatDate( $scope.contract.uptime);
        } else { //信息处理失败，提示错误信息
            rzhdialog(ngDialog,response.info,"error");
        }
    }).error(function (response) { //提交失败
...
//添加信息
App.controller("addcontractController", function ($scope, $http, $filter,ngDialog) {
    ...
    $scope.save = function () {
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            //增加分类属性
            $scope.contract.typecode=global_type_code;
            if($scope.contract.startime!=null&&$scope.contract.startime!=''){
                $scope.contract.startime = formatDayNum8($scope.contract.startime);
            }
            if($scope.contract.endtime!=null&&$scope.contract.endtime!=''){
                $scope.contract.endtime = formatDayNum8($scope.contract.endtime);
            }

            //封装来源名称
            angular.forEach($scope.sourcetreeInfos, function (data, index, array) {
                if($scope.contract.sourcecode==data.code){
                    $scope.contract.source=data.label;
                }
            });
            //封装合同模板路径
            angular.forEach($scope.tplcontracttreeInfos, function (data, index, array) {
                if($scope.contract.tplcode==data.code){
                    $scope.contract.attach=data.remark;
                }
            });

            var pData = {
                paras: JSON.stringify($scope.contract),
...
//修改信息
App.controller("updatecontractController", function ($scope, $http,$filter, ngDialog) {
    $('#endtime,#startime').datetimepicker({ //加载日期插件
        todayBtn:  1, // '今天'按钮显示
        autoclose: 1, //选择完成自动关闭
        minView: 2, //最小显示单位  2 代表到天
...
//修改状态
App.controller("changecontractController", function ($scope, $http,$filter, ngDialog) {
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        var table = $('#'+$scope.gridtableid).DataTable();
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            ...
            angular.forEach($scope.statustreeInfos, function (data, index, array) {
                if($scope.contract.statuscode==data.code){
                    $scope.contract.statusname=data.label;
                }
            });
            var pData = {
                paras: JSON.stringify($scope.contract)
