...
App.controller('bussreportController', ['$scope', '$http',"$stateParams", "ngDialog", function ($scope, $http,$stateParams, ngDialog) {
    global_type_code = null;
    $scope.gridtableid="bussreportTableGrids";
    $scope.global_type="day";//默认为日报表
    $scope.global_redirect_path="/";
    $scope.bussreport_treeshow=true;
    if(typeof($stateParams.path)!="undefined"&&$stateParams.path!=''){
        $scope.bussreport_treeshow=false;
        $scope.global_redirect_path="/"+$stateParams.path;
        if($stateParams.path.indexOf("bussinfo")>-1){
            global_type_code="bussinfo";
        }else
        if($stateParams.path.indexOf("plan")>-1){
            global_type_code="bussplan";
        }else
        if($stateParams.path.indexOf("contract")>-1){
            global_type_code="busscontract";
        }else
        if($stateParams.path.indexOf("cust")>-1){
            global_type_code="busscust";
        }else
        if($stateParams.path.indexOf("finance")>-1){
            global_type_code="bussfinance";
        }
    }
    //获取类型树信息
    //$.ajax({ //查询按钮权限
    ...
    //获取类型树信息
    $.ajax({ //查询按钮权限
        url: GserverURL+"/sys/dict/list?typecode=bussreport",
        method: 'POST',
        async: false
    }).success(function (response) {
        if (response.success) {
    ...
    $("#toadd").click(function () {
        ...
        $http({
            ...
            if (response.success) { //信息处理成功，进入用户中心页面
                ...
                window.location.href = "#/app/report/buss"+$scope.global_redirect_path+"/add";
            } else { //信息处理失败，提示错误信息
    ...
    $scope.toRemove=function(){
        window.location.href = "#/app/report/buss"+$scope.global_redirect_path;
    }
...
//类型树信息处理
App.controller('bussreportTypeController', ['$scope', "$http", function ($scope, $http) {
    $scope.show_all_info = true;
    $scope.all_info = "全部类型";
    $scope.showAllInfo = function(){
        global_type_code=null;
        $scope.toRemove();
        $("#bussreportTypePanel").find("li").removeClass("active"); //清除已选中
        var table = $('#'+$scope.gridtableid).DataTable();
        table.draw();
        ckClickTr($scope.gridtableid); //单击行，选中复选框
    }
...
//列表信息处理
App.controller('bussreportGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
    function retrieveData(sSource, aoData, fnCallback) {
        ...
        $.ajax({
            ...
            url: sSource,
            dataType: 'json',
            data: {"aoData": JSON.stringify(aoData),"typelike":"buss","typecode":global_type_code,"type":$scope.global_type}, //以json格式传递
            async: false,
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/report/buss"+$scope.global_redirect_path+"/view/"+data.id;
    });
    //文件下载
...
// 查看详情
App.controller("viewbussreportController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
        ...
        if (response.success) { //信息处理成功，进入用户中心页面
            $scope.bussreport = response.data;
        } else { //信息处理失败，提示错误信息
            rzhdialog(ngDialog,response.info,"error");
        }
    }).error(function (response) { //提交失败
...
//添加信息
App.controller("addbussreportController", function ($scope, $http, $filter, ngDialog) {
    $scope.bussreport = {};
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            if($scope.global_type=="all"){
                ...
                if($scope.starttime!=null&&$scope.starttime!=''){
                    $scope.bussreport.starttime = formatDayNum8($scope.starttime); //格式化开始日期
                }else{
                    isallselect=false;
                }
                if($scope.endtime!=null&&$scope.endtime!=''){
                    $scope.bussreport.endtime = formatDayNum8($scope.endtime); //格式化开始日期
                }else{
                    isallselect=false;
                }
                ...
                if($scope.reporttime!=null&&$scope.reporttime!=''){
                    $scope.bussreport.starttime = formatDayNum8($scope.reporttime); //格式化开始日期
                }else{
            ...
            //增加分类属性
            $scope.bussreport.typecode=global_type_code;
            $scope.bussreport.type=$scope.global_type;
            var pData = {
                paras: JSON.stringify($scope.bussreport),
                type:$scope.type
            };
           $http({
