...
App.controller('rscustodyController', ['$scope', '$http', "ngDialog", function ($scope, $http, ngDialog) {
    global_type_code = null;
    $scope.gridtableid="rscustodyTableGrids";
    ...
    function topbar_bind_operate(ngDialog){
        ...
        $("#todetails").click(function(){
            ...
            window.location.href = "#/app/la/rscustody/grain/"+global_type_code;
        });

        //生成报表
        $("#toreport").click(function(){
            window.location.href = "#/app/report/rsgain"+"/app-la-rscustody";
            return;
        });
    }
    //关闭弹出窗口操作
    $scope.toRemove=function(){
        window.location.href = "#/app/la/rscustody";
    }
...
//类型树信息处理
App.controller('rscustodyTypeController', ['$scope', "$http", function ($scope, $http) {
    $scope.show_all_info = true;
    $scope.all_info = "全部类型";
    $scope.showAllInfo = function(){
        global_type_code=null;
        $scope.toRemove();
        $("#rscustodyTypePanel").find("li").removeClass("active"); //清除已选中
        var table = $('#'+$scope.gridtableid).DataTable();
        table.draw();
        ckClickTr($scope.gridtableid); //单击行，选中复选框
    }
...
//列表信息处理
App.controller('rscustodyGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/la/rscustody/view/"+data.id;
    });
}]);
// 查看详情
    ...
    $http({
        ...
        if (response.success) { //信息处理成功，进入用户中心页面
            ...
            if($scope.grain!=null){
                ...
                //入仓时间
                $scope.grain.warehousetime=formatDate($scope.grain.warehousetime);
            }
        } else { //信息处理失败，提示错误信息
            rzhdialog(ngDialog,response.info,"error");
        }
