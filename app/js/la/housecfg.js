...
App.controller('housecfgController', ['$scope', '$http', "ngDialog", function ($scope, $http, ngDialog) {
    global_type_code = null;
    $scope.gridtableid="housecfgTableGrids";
    //获取仓房列表
    $.ajax({ //查询按钮权限
    ...
    //获取类型树信息
    $.ajax({
        url: GserverURL+"/sys/dict/list?typecode=houseconfig",
        method: 'POST',
        async: false
    }).success(function (response) {
        if (response.success) {
            $scope.housecfgtreeInfos = response.data;
        }
    });
    var base_levels = "";
    var levels = null;
    ...
    $scope.toRemove=function(){
        window.location.href = "#/app/la/housecfg";
    }
    //绑定topbar操作
    function topbar_bind_operate(ngDialog){
        ...
        $("#toadd").click(function () {
            ...
            window.location.href = "#/app/la/housecfg/add";
        });

        //修改
        $("#toupdate").click(function () {
            var housecfg=getTableData(table);
            if(housecfg==null){
                rzhdialog(ngDialog,"请先选择一条要修改的数据！","error");
                return;
            }
            $scope.housecfg=housecfg;
            window.location.href = "#/app/la/housecfg/update";
            $("#configtype").val(housecfg.configtype); //
        });
        ...
        $("#todelete").click(function () {
            ...
            ngDialog.openConfirm({
                template: 'delhousecfgDialogId',
                className: 'ngdialog-theme-default'
            }).then(function (value) { //用户点击确认
                $http({
                    url: '/la/housecfg/delete',
                    method: 'POST',
                    data: {id: housecfg.id}
...
//类型树信息处理
App.controller('housecfgTypeController', ['$scope', "$http", function ($scope, $http) {
    $scope.show_all_info = true;
    $scope.all_info = "所有仓房";
    $scope.showAllInfo = function(){
        global_type_code=null;
        $scope.toRemove();
        $("#housecfgTypePanel").find("li").removeClass("active"); //清除已选中
        var table = $('#'+$scope.gridtableid).DataTable();
        table.draw();
        ckClickTr($scope.gridtableid); //单击行，选中复选框
    }
...
//列表信息处理
App.controller('housecfgGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    //分页动态加载数据
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "la/housecfg/list",//获取数据的url
        fnServerData: retrieveData,           //获取数据的处理函数
        aoColumns: [
            {sTitle: getHtmlInfos("app/views/base/check_all.html"), width: "5%"},
            {sTitle: "序号", width: "8%"},
            {mDataProp: "configmark", sTitle: "配置类型"},
            {mDataProp: "buildyear", sTitle: "建设年份"},
            {mDataProp: "submitdate", sTitle: "交付日期"},
            {mDataProp: "devicefactory", sTitle: "厂商名称"},
            {mDataProp: "factorycontacts", sTitle: "联系人"},
            {mDataProp: "factoryphone", sTitle: "联系电话"},
            {
                mDataProp: "uptime",
                sTitle: "更新日期",
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/la/housecfg/view/"+data.id;
    });
}]);
// 查看详情
App.controller("viewhousecfgController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
            url: GserverURL+ '/la/housecfg/view',
        method: 'POST',
        data: {id: $stateParams.id}
    }).success(function (response) { //提交成功
        if (response.success) { //信息处理成功，进入用户中心页面
            $scope.housecfg = response.data;
        } else { //信息处理失败，提示错误信息
            rzhdialog(ngDialog,response.info,"error");
        }
    }).error(function (response) { //提交失败
...
//添加信息
App.controller("addhousecfgController", function ($scope, $http, ngDialog) {
    $scope.housecfg = null;
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            //增加仓房属性
            $scope.housecfg.housecode=global_type_code;
            angular.forEach($scope.treeInfos, function (data, index, array) {
                if($scope.housecfg.housecode==data.code){
                    $scope.housecfg.housename=data.label;
                }
            });
            //增加分类属性
            angular.forEach($scope.housecfgtreeInfos, function (data, index, array) {
                if($scope.housecfg.configtype==data.code){
                    $scope.housecfg.configmark=data.label;
                }
            });
            var pData = {
                paras: JSON.stringify($scope.housecfg)
            };
           $http({
                url: GserverURL+'/la/housecfg/add',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
...
//修改信息
App.controller("updatehousecfgController", function ($scope, $http, ngDialog) {
    ...
    $scope.save = function () {
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
           ...
           $http({
                url: GserverURL+'/la/housecfg/update',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
