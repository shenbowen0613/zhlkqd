...
App.controller('houseController', ['$scope', '$http', "ngDialog", function ($scope, $http, ngDialog) {
    ...
    $scope.toRemove=function(){
        window.location.href = "#/app/factory";
    }
...
//获取类型树信息
    ...
    function topbar_bind_operate(ngDialog){
        ...
        $("#toadd").click(function () {
            $scope.factory=null;
            window.location.href = "#/app/factory/add";
        });

        //修改
        $("#toupdate").click(function () {
            var factory=getTableData(table);
            if(factory==null){
                rzhdialog(ngDialog,"请先选择一条要修改的数据！","error");
                return;
            }
            $scope.factory=factory;
            window.location.href = "#/app/factory/update";
        });
...
//列表信息处理
App.controller('switchGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "la/switch/list",//获取数据的url
        fnServerData: retrieveData,           //获取数据的处理函数
        aoColumns: [
            {sTitle: getHtmlInfos("app/views/base/check_all.html"), width: "5%"},
            {sTitle: "序号", width: "8%"},
            {mDataProp: "housename", sTitle: "厂家名称"},
            {mDataProp: "ip", sTitle: "设备IP"},
            {mDataProp: "houst", sTitle: "端口号"},
            {mDataProp: "linecode", sTitle: "线路编码"},
            {mDataProp: "wincode", sTitle: "窗口编码"},
            {sTitle: "操作"}
        ],
        aoColumnDefs: [//设置列的属性
            {
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        //打开链接
        $http({
            url: GserverURL+'/la/switch/sendPost',
            method: 'POST',
            data: {"id":data.id,"postType":"1"}
...
// 查看详情
App.controller("viewFactoryController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
            url: GserverURL+ '/control/factory/view',
        method: 'POST',
...
//添加信息
App.controller("addFactoryController", function ($scope, $http, ngDialog) {
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        var table = $('#'+$scope.gridtableid).DataTable();
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            var pData = {
                paras: JSON.stringify($scope.factory)
            };
           $http({
                url: GserverURL+'/control/factory/add',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
...
//修改信息
App.controller("updateFactoryController", function ($scope, $http, ngDialog) {
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        var table = $('#'+$scope.gridtableid).DataTable();
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
           ...
           $http({
                url: GserverURL+'/control/factory/update',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
