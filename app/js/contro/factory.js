...
//列表信息处理
App.controller('factoryGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "control/factory/list",//获取数据的url
        fnServerData: retrieveData,           //获取数据的处理函数
        aoColumns: [
            {sTitle: getHtmlInfos("app/views/base/check_all.html"), width: "5%"},
            {sTitle: "序号", width: "8%"},
            {mDataProp: "firmcode", sTitle: "厂家编码"},
            {mDataProp: "firmname", sTitle: "厂家名称"},
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/factory/view/"+data.id;
    });
}]);
// 查看详情
