...
App.controller('relayController', ['$scope', '$http', "ngDialog", function ($scope, $http, ngDialog) {
    $scope.gridtableid="houseTableGrids";

    //关闭弹出窗口操作
    $scope.toRemove=function(){
        window.location.href = "#/app/contro_relay";
    }
...
//获取类型树信息
    ...
    function topbar_bind_operate(ngDialog){
        ...
        $("#toadd").click(function () {
            ...
            window.location.href = "#/app/contro_relay/add";
        });

        //修改
        $("#toupdate").click(function () {
            var relay=getTableData(table);
            if(relay==null){
            ...
            window.location.href = "#/app/contro_relay/update";
        });
...
//列表信息处理
App.controller('relayGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "/control/relay/list",//获取数据的url
        fnServerData: retrieveData,           //获取数据的处理函数
        aoColumns: [
            {sTitle: getHtmlInfos("app/views/base/check_all.html"), width: "5%"},
            {sTitle: "序号", width: "8%"},
            {mDataProp: "controlname", sTitle: "控制器"},
            {mDataProp: "relayno", sTitle: "继电器编号"},
            {mDataProp: "controlobj", sTitle: "控制对象"},
            {mDataProp: "objposition", sTitle: "对象位置"},
            {sTitle: "操作"}
        ],
        aoColumnDefs: [//设置列的属性
            {
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/contro_relay/view/"+data.id;
    });
}]);
// 查看详情
App.controller("viewRelayController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
            url: GserverURL+ '/control/relay/view',
        method: 'POST',
        data: {id: $stateParams.id}
    }).success(function (response) { //提交成功
        if (response.success) { //信息处理成功，进入用户中心页面
            $scope.relay = response.data;
        } else { //信息处理失败，提示错误信息
            rzhdialog(ngDialog,response.info,"error");
        }
    }).error(function (response) { //提交失败
...
//添加信息
App.controller("addRelayController", function ($scope, $http, ngDialog) {
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        var table = $('#'+$scope.gridtableid).DataTable();
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            $scope.relay.controlname = $("#controlcode").find("option:selected").text();
            var pData = {
                paras: JSON.stringify($scope.relay)
            };
           $http({
                url: GserverURL+'/control/relay/add',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
...
//修改信息
App.controller("updateRelayController", function ($scope, $http, ngDialog) {
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        var table = $('#'+$scope.gridtableid).DataTable();
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
           ...
           $http({
                url: GserverURL+'/control/relay/update',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
