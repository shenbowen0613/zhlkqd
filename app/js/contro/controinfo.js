...
App.controller('controlController', ['$scope', '$http', "ngDialog", function ($scope, $http, ngDialog) {
    $scope.gridtableid="houseTableGrids";

    //关闭弹出窗口操作
    $scope.toRemove=function(){
        window.location.href = "#/app/controinfo";
    }
...
//获取类型树信息
    ...
    function topbar_bind_operate(ngDialog){
        ...
        $("#toadd").click(function () {
            ...
            window.location.href = "#/app/controinfo/add";
        });

        //修改
        $("#toupdate").click(function () {
            var controlInfo=getTableData(table);
            if(controlInfo==null){
                rzhdialog(ngDialog,"请先选择一条要修改的数据！","error");
                return;
            }
            $scope.controlinfo=controlInfo;
            //获取厂家信息
            $.ajax({
                url: "/control/factory/allFirm",
                method: 'POST',
            ...
            window.location.href = "#/app/controinfo/update";
        });
...
//列表信息处理
App.controller('controlInfoGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "control/controlinfo/list",//获取数据的url
        fnServerData: retrieveData,           //获取数据的处理函数
        aoColumns: [
            {sTitle: getHtmlInfos("app/views/base/check_all.html"), width: "5%"},
            {sTitle: "序号", width: "8%"},
            {mDataProp: "controlcode", sTitle: "控制器编码"},
            {mDataProp: "controlname", sTitle: "控制器名称"},
            {mDataProp: "ip", sTitle: "ip地址"},
            {mDataProp: "port", sTitle: "端口"},
    ...
    $("#"+$scope.gridtableid+" tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/app/controinfo/view/"+data.id;
    });
}]);
// 查看详情
App.controller("viewControlInfoController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
            url: GserverURL+ '/control/controlinfo/view',
        method: 'POST',
        data: {id: $stateParams.id}
    }).success(function (response) { //提交成功
        if (response.success) { //信息处理成功，进入用户中心页面
            $scope.controlinfo = response.data;
        } else { //信息处理失败，提示错误信息
            rzhdialog(ngDialog,response.info,"error");
        }
    }).error(function (response) { //提交失败
...
//添加信息
App.controller("addControlInfoController", function ($scope, $http, ngDialog) {
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        var table = $('#'+$scope.gridtableid).DataTable();
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
            $scope.controlinfo.firmname = $("#firmcode").find("option:selected").text();
            var pData = {
                paras: JSON.stringify($scope.controlinfo)
            };
           $http({
                url: GserverURL+'/control/controlinfo/add',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
...
//修改信息
App.controller("updateControlInfoController", function ($scope, $http, ngDialog) {
    //添加数据
    $scope.save = function () {
        angularParamString($http); //解决post提交接收问题，json方式改为string方式
        var table = $('#'+$scope.gridtableid).DataTable();
        ...
        if(false) {rzhdialog(ngDialog,"您输入的表单内容的不合法，请修改后提交","error");return;}else{
           ...
           $http({
                url: GserverURL+'/control/controlinfo/update',
                method: 'POST',
                data: pData
            }).success(function (response) { //提交成功
                if (response.success) { //信息处理成功，进入用户中心页面
