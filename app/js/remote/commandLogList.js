...
App.controller('commandLogListController', ['$scope', '$http',"$stateParams", "ngDialog", function ($scope, $http,$stateParams, ngDialog) {
    global_type_code = null;
    $scope.gridtableid="commandLogListTableGrids";
    angularParamString($http); //解决post提交接收问题，json方式改为string方式
}]);

//列表信息处理
App.controller('commandLogListGridController', ['$scope', "$http", "ngDialog", function ($scope,$stateParams, $http, ngDialog) {
    //分页动态加载数据
    ...
    var table = $('#'+$scope.gridtableid).DataTable({
        ...
        ajaxSource: "remote/log/uploadLog",//获取数据的url
        fnServerData: retrieveData,           //获取数据的处理函数
        aoColumns: [
            //{sTitle: getHtmlInfos("app/views/base/check_all.html"), width: "5%"},
            {sTitle: "序号", width: "8%"},
            {
                mDataProp: "commandname",
                sTitle: "命令名称"
            },
            {
                mDataProp: "commandcontent",
                sTitle: "命令内容"

            },

            {mDataProp: "execptioninfo", sTitle: "异常信息"},
            {
                mDataProp: "crtime",
                sTitle: "时间",
...
// 查看详情
App.controller("viewcommandLogListController", function ($scope, $stateParams,$http, ngDialog) {
    $http({
        ...
        if (response.success) { //信息处理成功，进入用户中心页面
            $scope.commandLogList = response.data;
            $scope.commandLogList.starttime=formatDate( $scope.commandLogList.starttime);
            $scope.commandLogList.endtime=formatDate( $scope.commandLogList.endtime);
        } else { //信息处理失败，提示错误信息
