...
var selHouseCode;
var selHouseName;
App.controller('outDetailedController', ['$scope', '$http', "$stateParams", "ngDialog", function ($scope, $http, $stateParams, ngDialog) {
    global_type_code = null;
    $scope.gridtableid = "outDetailedTableGrids";
    angularParamString($http); //解决post提交接收问题，json方式改为string方式
}]);

//列表信息处理$scope, $http, $filter, ngDialog
App.controller('outDetailedGridController', ['$scope', "$http", "ngDialog", function ($scope, $http, ngDialog) {
    ...
    var table = $('#' + $scope.gridtableid).DataTable({
        ...
        aoColumns: [
            ...
            // {sTitle: "检验结果", mDataProp: "outinQualityResult.advise"},
            {
            ...
            // {sTitle: "质检扣量(%)", mDataProp: "outinQualityResult.qualitycutweight"},
            // {sTitle: "现场扣量(千克)", mDataProp: "outinTare.cutweight"},
            // {sTitle: "净重(千克)", mDataProp: "outinTare.netweight"},
            {
            ...
            // {sTitle: "结算单价(元/千克)", mDataProp: "outinSettlement.jsprice"},
            // {sTitle: "结算金额(元)", mDataProp: "outinSettlement.jsmoney"},
            {sTitle: "操作"}
        ],
        aoColumnDefs: [//设置列的属性
            //{
    ...
    $("#" + $scope.gridtableid + " tbody").on('click', 'tr td button.toview', function () {
        ...
        window.location.href = "#/outin/out_detailed/detail/" + data.busno;
    });
...
App.controller("viewOutinDetailController", function ($scope, $stateParams, $http, ngDialog) {
    ...
    $scope.toRemove = function () {
        window.location.href = "#/outin/out_detailed";
    }
    ...
    $http({
        ...
        if (response.success) { //信息处理成功，进入用户中心页面
            ...
            if($scope.outinEnty.outinPincard){
            ...
            console.log( $scope.jiesuandanjia );
        } else { //信息处理失败，提示错误信息
